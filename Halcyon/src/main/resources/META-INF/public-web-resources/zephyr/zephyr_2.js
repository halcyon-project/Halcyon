...
function DrawAxis(scene) {
    ...
    const points = [];
    points.push( new Vector3( -200000, 0, 0 ) );
    points.push( new Vector3( 200000, 0, 0 ) );
    const geometry = new BufferGeometry().setFromPoints( points );
    const line = new Line( geometry, material );
    scene.add(line);
    const points2 = [];
    points2.push( new Vector3( 0, -200000, 0 ) );
    points2.push( new Vector3( 0, 200000, 0 ) );
          ...
          if (this.level < 4) {
            ...
            //if (w<0) {
                if (!this.booted) {          
                    ...
                    const nw = new ImageViewer(renderer, url, x, y, Math.min(offm,w), Math.min(offm,h), tilex, tiley, offset, info, nextlevel );
                    const ww = x+offm;
                    const hh = y+offm;
                    const ha = ((Math.min(offm,h) + (h - hh))/2) - (offm/2);

                    nw.position.set( - Math.min(offm,w) / 2, ha, 0);
                    high.add(nw);
                    console.log("INW : "+ x+" " +y+" "+offm+" "+offm);

                    if (ww<=w) {
                        const ne = new ImageViewer(renderer, url, x + offm, y, w - ww, Math.min(offm,h), tilex, tiley, offset, info, nextlevel );
                        ne.position.set( (( w - ww ) / 2) + 1000, ha, 0);
                        high.add(ne);
                        console.log("INE : "+(x+offm)+" "+  y+" "+(w -    offm)+" "+   offm);
                    }
                    if (hh<=h) {
                        const sw = new ImageViewer(renderer, url, x, y + offm, Math.min(offm,w), h - hh, tilex, tiley, offset, info, nextlevel );
                        sw.position.set(     - Math.min(offm,w)   / 2, - offm / 2, 0);
                        high.add(sw);
                        console.log("ISW : "+ x+" "      + (y+offm)+" "+   offm +" "+(h-offm));
                    }
                    if ((ww<=w)&&(hh<=h)) {
                        const se = new ImageViewer(renderer, url, x + offm, y + offm, w - ww, h - hh, tilex, tiley, offset, info, nextlevel );
                        se.position.set( ((  w - ww ) / 2) + 1000, -  offm / 2, 0);
                        high.add(se);
                        console.log("ISE : "+(x+offm)+" "+ (y+offm)+" "+(w-offm)+" "+(h-offm));
                    }
                    ...
                    if (w<=tilex) {
                        ...
                        const sigh = 0.55 * this.edistance ;
