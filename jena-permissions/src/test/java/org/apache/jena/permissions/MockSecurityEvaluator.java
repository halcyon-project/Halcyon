...
public class MockSecurityEvaluator implements SecurityEvaluator {
    ...
    public MockSecurityEvaluator(final boolean loggedIn, final boolean create, final boolean read, final boolean update,
            final boolean delete, final boolean forceTripleChecks, boolean hardReadError) {
    ...
    public boolean evaluate(final Object principal, final Action action, final Node graphIRI, final Triple triple) {
        if (forceTripleChecks) {
            if (triple.getSubject().equals(Node.ANY) || triple.getPredicate().equals(Node.ANY)
    ...
    public String toString() {
        return String.format("C:%s R:%s U:%s D:%s force:%s hardRead:%s", create, read, update, delete,
                forceTripleChecks, hardReadError);
    }
